...
OUTPUT_DIR="evidence/chaos/db-down/${TIMESTAMP}"
...
REGISTER_PROBES_FILE="${OUTPUT_DIR}/api-register-probes.csv"
CHAOS_RUNNER_LOG="${OUTPUT_DIR}/db-down-runner.log"
RECOVERY_SUMMARY_FILE="${OUTPUT_DIR}/recovery-summary.txt"
...
unique_email() {
  printf 'chaos_db_%s_%s@test.local' "${TIMESTAMP}" "$(date +"%s")$RANDOM"
}
...
capture_system_snapshots() {
  ...
  lsof -i :"${DB_PORT}" > "${OUTPUT_DIR}/lsof-db-port-${DB_PORT}-${phase}.txt" 2>&1 || true
  lsof -i :8080 > "${OUTPUT_DIR}/lsof-backend-port-8080-${phase}.txt" 2>&1 || true
}

write_metadata() {
  cat > "${METADATA_FILE}" <<EOF
scenario name: C2 DB Down
db port: ${DB_PORT}
...
top-pre.txt, vm-stat-pre.txt, lsof-db-port-${DB_PORT}-pre.txt, lsof-backend-port-8080-pre.txt
Pre-fault system and port snapshots.

top-post.txt, vm-stat-post.txt, lsof-db-port-${DB_PORT}-post.txt, lsof-backend-port-8080-post.txt
...
probe_register() {
  ...
  payload="$(printf '{"email":"%s","password":"secret123","name":"Chaos User"}' "$(unique_email)")"
  result="$(curl -sS -o /dev/null -w '%{http_code},%{time_total}' \
    -H 'Content-Type: application/json' \
    -X POST \
    -d "${payload}" \
  ...
  append_probe_result "${REGISTER_PROBES_FILE}" "${phase}" "${url}" "${http_status}" "${total_time}" "${succeeded}"
  [[ "${succeeded}" == "true" ]]
}
...
run_recovery_probes() {
  ...
  while (( elapsed <= 90 )); do
    ...
    if probe_register "recovery"; then
      register_ok="true"
    fi

    if [[ "${products_ok}" == "true" && "${login_ok}" == "true" && "${register_ok}" == "true" ]]; then
      printf 'All recovery probes succeeded at %s\n' "$(timestamp)" > "${RECOVERY_SUMMARY_FILE}"
      return 0
    fi
  ...
  printf 'Recovery probes did not fully succeed within 90 seconds after downtime at %s\n' "$(timestamp)" > "${RECOVERY_SUMMARY_FILE}"
  return 1
}
...
start_fault_injection() {
  DB_PORT="${DB_PORT}" DOWNTIME_SECONDS="${DOWNTIME_SECONDS}" \
    ./chaos/db-down.sh > "${CHAOS_RUNNER_LOG}" 2>&1 &
  CHAOS_PID=$!
}
...
echo "DB downtime window ended."
echo "Manual DB restart is required before recovery can succeed."
